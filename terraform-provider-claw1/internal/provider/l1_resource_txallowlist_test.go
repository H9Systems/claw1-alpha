...
func TestInjectGenesisPrecompiles_withWarp(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	admin := "0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC"
	if err := injectGenesisPrecompiles("demo", admin, true); err != nil {
		t.Fatalf("injectGenesisPrecompiles returned error: %v", err)
	}
	...
	txAllowList, ok := config["txAllowListConfig"].(map[string]interface{})
	if !ok {
		t.Fatal("txAllowListConfig missing under config")
	}
	admins := txAllowList["adminAddresses"].([]interface{})
	if len(admins) != 1 || admins[0] != admin {
		t.Fatalf("adminAddresses = %#v", admins)
	...
	warpCfg, ok := config["warpConfig"].(map[string]interface{})
	if !ok {
		t.Fatal("warpConfig missing under config when injectWarp=true")
	}
	if warpCfg["blockTimestamp"].(float64) != 0 {
		t.Fatalf("warpConfig.blockTimestamp = %v, want 0", warpCfg["blockTimestamp"])
	}
	if warpCfg["quorumNumerator"].(float64) != 0 {
		t.Fatalf("warpConfig.quorumNumerator = %v, want 0", warpCfg["quorumNumerator"])
	}
}
