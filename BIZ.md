...
La infraestructura es de código abierto (Apache 2.0). ⚠️ **Nota sobre el modelo de negocio:** lo que sigue sobre el modelo de negocio no es una declaración oficial y podría cambiar por completo.
...
Precio objetivo: *por definir.* El ancla de precio es el costo de que una institución construya y audite esto de forma independiente (meses de investigación legal + auditoría externa).
...
Primeros compromisos de servicios profesionales: *por definir.* Retainer de soporte: *por definir.*

---

## Supuestos de Ingresos

> ⚠️ Las proyecciones de ingresos no son declaraciones oficiales y podrían cambiar por completo.

| Escenario | Clientes Año 1 | Ingresos |
|----------|----------------|---------|
| Conservador | 1 cliente ancla | *por definir* |
| Base | 3 licencias enterprise + 2 compromisos PS | *por definir* |
| Optimista | 8 licencias enterprise + soporte recurrente | *por definir* |

El año 1 se trata de aprender qué necesitan realmente los clientes del nivel pago, no de optimizar ingresos. El hito que desbloquea las siguientes etapas es: una institución financiera LATAM ejecutando `terraform apply` en producción y pagando por la biblioteca de contratos.
...
2. El rastro de evidencias `ComplianceRegistry`: una vez que el historial de cumplimiento de una institución vive en la cadena, cambiar significa reconstruir ese rastro desde cero
...
**Canal: ecosistema de infraestructura cloud.** La configuración OCI Terraform es un constructor deliberado de relaciones con proveedores de infraestructura.

**Canal: ecosistema Avalanche.** Un proveedor de L1 enfocado en cumplimiento usando su toolchain encaja en su narrativa empresarial.
...
| Intro socio cloud | Introducción al equipo de servicios financieros | Semana 4 |
...
**Riesgo técnico**: La biblioteca de contratos de cumplimiento (ERC-3643 + eERC + ICTT bridge) es más compleja que el MVP inicial. La auditoría externa de contratos inteligentes es un prerequisito para el nivel pago; el costo y el cronograma de auditoría son incógnitas.
...
2. **El rastro de evidencias es el foso.** Los registros de `ComplianceRegistry` deben ser inmutables y consultables. No rediseñes el modelo de datos sin entender qué consultará un auditor.
...
4. **El ICP paga por simplificar el cumplimiento blockchain.** Cada decisión de cumplimiento específica por jurisdicción codificada en el producto reemplaza trabajo que el cliente de otra forma tendría que externalizar.
